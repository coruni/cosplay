...
class SeriesListDialog(QDialog):
    """显示系列列表（含关联角色数），可搜索、可点选填入主窗口。"""

    def __init__(self, series_items: list[dict], parent=None):
        super().__init__(parent)
        self.setWindowTitle('系列列表')
        self.resize(420, 560)
        self.setAttribute(Qt.WA_StyledBackground, True)
        self.setObjectName('SettingsDialog')
        self._all = series_items or []
        self._selected_name = ''
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        header.setSpacing(8)
        title = QLabel(f'系列列表（{len(self._all)}）')
        ...
        self.search_edit.setPlaceholderText('搜索系列名称...')
    ...
    def _populate(self, filter_text: str):
        ...
        for s in self._all:
            name = s.get('series', '')
            count = s.get('galleryCount', 0)
            n_chars = len(s.get('characters', []) or [])
            if flt and flt not in name.lower():
                continue
            item = QListWidgetItem()
            char_suffix = f' · {n_chars} 个角色' if n_chars else ''
            item.setText(f'{name}   ·   {count} 个图包{char_suffix}')
...
class CharacterListDialog(QDialog):
    """显示角色列表（已按当前所选系列过滤），可搜索、可点选填入主窗口。"""

    def __init__(self, char_items: list[dict], parent=None):
        super().__init__(parent)
        self.setWindowTitle('角色列表')
        self.resize(420, 560)
        self.setAttribute(Qt.WA_StyledBackground, True)
        self.setObjectName('SettingsDialog')
        self._all = char_items or []
        self._selected_name = ''
    ...
    def _build_ui(self):
        ...
        header = QHBoxLayout()
        header.setSpacing(8)
        title = QLabel(f'角色列表（{len(self._all)}）')
        title.setObjectName('settingsSection')
        title.setFont(QFont('Segoe UI', 11, QFont.Bold))
        header.addWidget(title)
        header.addStretch()
